...
class ToolServiceClient:
    ...
    async def connect(self):
        ...
        self.websocket = await websockets.connect(
            self.hub_url, ping_interval=None  # 我们自己发送心跳
        )
    ...
    async def _process_message(self, raw_message: str):
        ...
        try:
            ...
            if msg_type == "registered":
                ...
                print(
                    f"[Client] Registered! service_id={self.service_id}, tunnel_id={self.tunnel_id}"
                )
    ...
    async def call_remote_service(
        self, tunnel_id: str, method: str, params: dict, timeout: float = 30.0
    ) -> dict:
        """
        调用远程服务 (通过云端代理)
...
async def run_service(
    ...
    **kwargs,
    ...
    """快速启动一个工具服务"""
    runner = LocalServiceRunner(name=name, description=description, hub_url=hub_url, **kwargs)
